...
func TestRelativeScoreFusion(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := ScoreFusion(tt.hits, tt.weights, tt.windowSize, tt.numKNNQueries, false, false); !compareFusionResults(got, tt.want) {
...
func TestDistributionBasedScoreFusion(t *testing.T) {
	tests := []struct {
		name          string
		hits          search.DocumentMatchCollection
		weights       []float64
		...
		},
		{
			...
			want: FusionResult{
				Hits: search.DocumentMatchCollection{
					{ID: "b", Score: 0.622}, // FTS: (0.8-0.555)/(1.045-0.555)*0.4 + KNN: (0.9-0.555)/(1.045-0.555)*0.6 ≈ 0.622
					{ID: "a", Score: 0.582}, // FTS: (0.9-0.555)/(1.045-0.555)*0.4 + KNN: (0.8-0.555)/(1.045-0.555)*0.6 ≈ 0.582
					{ID: "c", Score: 0.296}, // FTS: (0.7-0.555)/(1.045-0.555)*0.4 + KNN: (0.7-0.555)/(1.045-0.555)*0.6 ≈ 0.296
				},
				Total:    3,
				MaxScore: 0.622,
			},
		},
		{
			...
			want: FusionResult{
				Hits: search.DocumentMatchCollection{
					{ID: "a", Score: 0.561}, // DBSF: FTS=0.704×0.3 + KNN0=0.5×0.4 + KNN1=0.5×0.3 = 0.561
					{ID: "b", Score: 0.520}, // DBSF: FTS=0.5×0.3 + KNN0=0.704×0.4 + KNN1=0.296×0.3 = 0.520
					{ID: "c", Score: 0.418}, // DBSF: FTS=0.296×0.3 + KNN0=0.296×0.4 + KNN1=0.704×0.3 = 0.418
				},
				Total:    3,
				MaxScore: 0.561,
			},
		},
		{
			...
			want: FusionResult{
				Hits: search.DocumentMatchCollection{
					{ID: "a", Score: 1.0}, // When std dev is 0, max == min, so normalized score becomes 1.0
					{ID: "b", Score: 1.0},
					{ID: "c", Score: 1.0},
				},
				Total:    3,
		...
		},
		{
			...
			want: FusionResult{
				Hits: search.DocumentMatchCollection{
					{ID: "b", Score: 0.533}, // DBSF: FTS=0.333×0.4 + KNN0=0.667×0.6 = 0.533 (b is in both top 2 FTS and KNN0)
					{ID: "a", Score: 0.267}, // DBSF: FTS=0.667×0.4 + KNN0=0×0.6 = 0.267 (a is in top 2 FTS but not KNN0)
				},
				Total:    2,
				MaxScore: 0.533,
			},
		},
		{
			...
			hits: search.DocumentMatchCollection{
				{ID: "a", Score: 0.9, ScoreBreakdown: map[int]float64{0: 0.8}},
				{ID: "b", Score: 0.8, ScoreBreakdown: map[int]float64{1: 0.7}},
				{ID: "c", Score: 0.7, ScoreBreakdown: map[int]float64{0: 0.6, 1: 0.9}},
				{ID: "d", Score: 0.6, ScoreBreakdown: map[int]float64{}},
			},
			weights:       []float64{0.3, 0.4, 0.3},
			windowSize:    4,
			numKNNQueries: 2,
			want: FusionResult{
				Hits: search.DocumentMatchCollection{
					{ID: "a", Score: 0.484}, // DBSF: FTS=0.724×0.3 + KNN0=0.667×0.4 + KNN1=0×0.3 = 0.484
					{ID: "c", Score: 0.461}, // DBSF: FTS=0.425×0.3 + KNN0=0.333×0.4 + KNN1=0.667×0.3 = 0.461
					{ID: "b", Score: 0.272}, // DBSF: FTS=0.575×0.3 + KNN0=0×0.4 + KNN1=0.333×0.3 = 0.272
					{ID: "d", Score: 0.083}, // DBSF: FTS=0.276×0.3 + KNN0=0×0.4 + KNN1=0×0.3 = 0.083
				},
				Total:    4,
				MaxScore: 0.484,
			},
		},
		{
			name: "documents with only KNN scores",
			hits: search.DocumentMatchCollection{
				{ID: "a", Score: 0.0, ScoreBreakdown: map[int]float64{0: 0.9}},
				{ID: "b", Score: 0.0, ScoreBreakdown: map[int]float64{1: 0.8}},
				{ID: "c", Score: 0.0, ScoreBreakdown: map[int]float64{0: 0.7, 1: 0.6}},
			},
			...
			want: FusionResult{
				Hits: search.DocumentMatchCollection{
					{ID: "a", Score: 0.2},   // DBSF: FTS=0×0.5 + KNN0=0.667×0.3 + KNN1=0×0.2 = 0.2
					{ID: "c", Score: 0.167}, // DBSF: FTS=0×0.5 + KNN0=0.333×0.3 + KNN1=0.333×0.2 = 0.167
					{ID: "b", Score: 0.133}, // DBSF: FTS=0×0.5 + KNN0=0×0.3 + KNN1=0.667×0.2 = 0.133
				},
				Total:    3,
				MaxScore: 0.2,
			},
		},
		{
			name: "mixed scenario with different score ranges",
			hits: search.DocumentMatchCollection{
				{ID: "a", Score: 1.0, ScoreBreakdown: map[int]float64{0: 0.1}},
				{ID: "b", Score: 0.1, ScoreBreakdown: map[int]float64{0: 1.0}},
				{ID: "c", Score: 0.5, ScoreBreakdown: map[int]float64{0: 0.5}},
			},
			weights:       []float64{0.5, 0.5},
			windowSize:    3,
			numKNNQueries: 1,
			want: FusionResult{
				Hits: search.DocumentMatchCollection{
					{ID: "a", Score: 0.508}, // DBSF: FTS=0.711×0.5 + KNN0=0.304×0.5 = 0.508
					{ID: "b", Score: 0.508}, // DBSF: FTS=0.304×0.5 + KNN0=0.711×0.5 = 0.508
					{ID: "c", Score: 0.485}, // DBSF: FTS=0.485×0.5 + KNN0=0.485×0.5 = 0.485
				},
				Total:    3,
				MaxScore: 0.508,
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := ScoreFusion(tt.hits, tt.weights, tt.windowSize, tt.numKNNQueries, true, false); !compareFusionResults(got, tt.want) {
				t.Errorf("DistributionBasedScoreFusion() = %v, want %v", got, tt.want)
				// Print detailed comparison for debugging
				t.Logf("Got hits:")
				for i, hit := range got.Hits {
					t.Logf("  [%d] ID: %s, Score: %.6f", i, hit.ID, hit.Score)

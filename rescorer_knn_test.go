...
func createHybridSearchRequest(scoreMethod string) *SearchRequest {
...
func TestRRFEndToEnd(t *testing.T) {
	...
	searchRequest := createHybridSearchRequest(ScoreRRF)
...
func TestRRFPagination(t *testing.T) {
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			firstPageRequest := createHybridSearchRequest(ScoreRRF)
			...
			secondPageRequest := createHybridSearchRequest(ScoreRRF)
...
func TestRRFFaceting(t *testing.T) {
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			rrfRequest := createHybridSearchRequest(ScoreRRF)
...
// verifyDBSFResults verifies that the search hits match expected DBSF ranking and scores
func verifyDBSFResults(t *testing.T, hits search.DocumentMatchCollection) {
	// For DBSF, we expect similar high-level results to RSF but with distribution-based normalization
	// DBSF uses mean ± 3*standard_deviation for min/max calculation
	// Expected top documents should include those matching "dark" query and similar vectors

	// Verify we have reasonable number of results
	if len(hits) == 0 {
		t.Fatal("Expected non-empty search results for DBSF")
	}

	// Verify we have at least 8 results
	if len(hits) < 8 {
		t.Errorf("Expected at least 8 results for DBSF, got %d", len(hits))
	}
	...
	// Verify that "dark blue" appears in top 3 positions (high text + vector relevance)
	if pos, found := docMap["dark blue"]; !found {
		t.Error("Expected 'dark blue' to appear in results but not found")
	} else if pos >= 5 {
		t.Errorf("Expected 'dark blue' in top 3 positions, found at position %d", pos+1)
	...
	// Verify scores are reasonable and within expected range
	// DBSF scores should be between 0 and sum of weights (3.0 with default weights)
	// but typically should be more constrained than the full range
	for i, hit := range hits {
		if hit.Score < 0 || hit.Score > 3.0 {
			t.Errorf("Hit %d (%s) has unreasonable score: %.6f", i, hit.ID, hit.Score)
...
// TestDBSFEndToEnd tests DBSF scoring with a single index
func TestDBSFEndToEnd(t *testing.T) {
	// Setup the index configuration
	index, cleanup := setupSingleIndex(t)
	defer cleanup()

	// Create the search request
	searchRequest := createHybridSearchRequest(ScoreDBSF)
	...
	// Verify DBSF results
	verifyDBSFResults(t, result.Hits)
}

// TestDBSFAliasWithSingleIndex tests DBSF with an alias containing one index
func TestDBSFAliasWithSingleIndex(t *testing.T) {
	// Setup the alias configuration
	alias, cleanup := setupAliasWithSingleIndex(t)
	defer cleanup()
	...
	// Verify DBSF results - should be identical to direct index search
	verifyDBSFResults(t, result.Hits)
}

// TestDBSFAliasWithTwoIndexes tests DBSF with an alias containing two indexes
func TestDBSFAliasWithTwoIndexes(t *testing.T) {
	// Setup the alias configuration
	alias, cleanup := setupAliasWithTwoIndexes(t)
	defer cleanup()
	...
	// Verify DBSF results - should be identical to single index results
	verifyDBSFResults(t, result.Hits)
}

// TestDBSFNestedAliases tests DBSF with an alias containing two index aliases
func TestDBSFNestedAliases(t *testing.T) {
	// Setup the nested aliases configuration
	masterAlias, cleanup := setupNestedAliases(t)
	defer cleanup()
...
// TestDBSFPagination tests DBSF with pagination across different index/alias configurations
func TestDBSFPagination(t *testing.T) {
	scenarios := []struct {
		name  string
		setup func(t *testing.T) (Index, func())
	}{
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			// Create first page request (first 5 results)
			firstPageRequest := createHybridSearchRequest(ScoreDBSF)
			firstPageRequest.From = 0
			firstPageRequest.Size = 5
			...
			// Create second page request (next 5 results, starting from index 5)
			secondPageRequest := createHybridSearchRequest(ScoreDBSF)
			secondPageRequest.From = 5
			secondPageRequest.Size = 5
			...
			// Verify combined DBSF results
			verifyDBSFResults(t, combinedHits)
		})
	}
}
...
// TestDBSFFaceting tests that facet results are identical whether using DBSF or default scoring in hybrid search
func TestDBSFFaceting(t *testing.T) {
	scenarios := []struct {
		name  string
		setup func(t *testing.T) (Index, func())
	}{
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			// Create search request with DBSF scoring and identical facets
			dbsfRequest := createHybridSearchRequest(ScoreDBSF)
			dbsfRequest.Size = 10
			// Add identical facet for color field with size 10
			colorFacetDBSF := NewFacetRequest("color", 10)
			dbsfRequest.AddFacet("color", colorFacetDBSF)
			...
			dbsfResult, err := index.Search(dbsfRequest)
			if err != nil {
				t.Fatalf("DBSF scoring search failed: %v", err)
			}
			...
			if len(dbsfResult.Hits) == 0 {
				t.Fatal("Expected search results with DBSF scoring, got none")
			}
			...
			if dbsfResult.Facets == nil {
				t.Fatal("Expected facets with DBSF scoring, got nil")
			}

			// Check that color facet exists in both results
			defaultColorFacet, defaultExists := defaultResult.Facets["color"]
			dbsfColorFacet, dbsfExists := dbsfResult.Facets["color"]

			if !defaultExists {
				t.Fatal("Expected color facet in default scoring results")
			}
			if !dbsfExists {
				t.Fatal("Expected color facet in DBSF scoring results")
			}

			// Compare the facet results - they should be identical
			// Since facets are based on the document corpus and not scoring,
			// they should not be affected by the scoring method (even with KNN)
			if defaultColorFacet.Total != dbsfColorFacet.Total {
				t.Errorf("Facet totals differ: default=%d, DBSF=%d",
					defaultColorFacet.Total, dbsfColorFacet.Total)
			}

			if defaultColorFacet.Missing != dbsfColorFacet.Missing {
				t.Errorf("Facet missing counts differ: default=%d, DBSF=%d",
					defaultColorFacet.Missing, dbsfColorFacet.Missing)
			}

			if defaultColorFacet.Other != dbsfColorFacet.Other {
				t.Errorf("Facet other counts differ: default=%d, DBSF=%d",
					defaultColorFacet.Other, dbsfColorFacet.Other)
			}

			// Compare the facet terms
			defaultTerms := defaultColorFacet.Terms.Terms()
			dbsfTerms := dbsfColorFacet.Terms.Terms()

			if len(defaultTerms) != len(dbsfTerms) {
				t.Errorf("Facet terms count differs: default=%d, DBSF=%d",
					len(defaultTerms), len(dbsfTerms))
			} else {
				// Compare each term
				for i, defaultTerm := range defaultTerms {
					dbsfTerm := dbsfTerms[i]
					if defaultTerm.Term != dbsfTerm.Term {
						t.Errorf("Facet term differs at position %d: default=%s, DBSF=%s",
							i, defaultTerm.Term, dbsfTerm.Term)
					}
					if defaultTerm.Count != dbsfTerm.Count {
						t.Errorf("Facet term count differs for %s: default=%d, DBSF=%d",
							defaultTerm.Term, defaultTerm.Count, dbsfTerm.Count)
					}
				}
			}
		})

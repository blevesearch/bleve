...
func (s *Scorch) pausePersisterForMergerCatchUp(lastPersistedEpoch uint64, lastMergedEpoch uint64,
	...
	// on finding fewer files on disk, persister pauses for a bit -
	// for sufficient in-memory segments to pile up for the next
	// memory merge and persist loop;
	// on finding too many files on disk, persister waits until the merger
	// catches up to reduce the segment file count to under the threshold.
	numFilesOnDisk, _ := s.diskFileStats()
	if numFilesOnDisk < uint64(po.PersisterNapUnderNumFiles) && po.PersisterNapTimeMSec > 0 {
		select {
		case <- s.closeCh:

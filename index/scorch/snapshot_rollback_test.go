...
func TestIndexRollback(t *testing.T) {
	...
	prev, err := sh.SnapshotPrevious(ss)
	if err != nil {
		t.Error(err)
	}
	...
	if len(newRoot.segment) != len(prev.segment) {
		t.Errorf("Unexpected number segments, %v != %v", len(newRoot.segment), len(prev.segment))
	}
	if len(newRoot.offsets) != len(prev.offsets) {
		t.Errorf("Unexpected number offsets, %v != %v", len(newRoot.offsets), len(prev.offsets))
	}
	for i, v := range newRoot.offsets {
		if v != prev.offsets[i] {
			t.Errorf("Unexpected offset, %v != %v", v, prev.offsets[i])
		}
	}
}

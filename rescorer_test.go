...
// verifyFTSDBSFResults verifies that the search hits match expected DBSF ranking and scores for FTS-only search
func verifyFTSDBSFResults(t *testing.T, hits search.DocumentMatchCollection) {
	// For FTS DBSF, we expect documents that match multiple query components to rank higher
	// Query components: "dark" in color, "light" in description, "blue" in category
	// DBSF uses distribution-based normalization (mean ± 3*standard_deviation)

	// Verify we have reasonable number of results
	if len(hits) == 0 {
		t.Fatal("Expected non-empty search results for FTS DBSF")
	}

	// Verify we have at least 5 results for meaningful comparison
	if len(hits) < 5 {
		t.Errorf("Expected at least 5 results for FTS DBSF, got %d", len(hits))
	}
...
// TestFTSDBSFEndToEnd tests DBSF scoring with a single FTS index
func TestFTSDBSFEndToEnd(t *testing.T) {
	// Setup the index configuration
	index, cleanup := setupFTSSingleIndex(t)
	defer cleanup()

	// Create the search request
	searchRequest := createFTSSearchRequest(ScoreDBSF)
	...
	// Verify FTS DBSF results
	verifyFTSDBSFResults(t, result.Hits)
}

// TestFTSDBSFAliasWithSingleIndex tests DBSF with an alias containing one FTS index
func TestFTSDBSFAliasWithSingleIndex(t *testing.T) {
	// Setup the alias configuration
	alias, cleanup := setupFTSAliasWithSingleIndex(t)
	defer cleanup()
	...
	// Verify FTS DBSF results - should be identical to direct index search
	verifyFTSDBSFResults(t, result.Hits)
}

// TestFTSDBSFAliasWithTwoIndexes tests DBSF with an alias containing two FTS indexes
func TestFTSDBSFAliasWithTwoIndexes(t *testing.T) {
	// Setup the alias configuration
	alias, cleanup := setupFTSAliasWithTwoIndexes(t)
	defer cleanup()
	...
	// Verify FTS DBSF results - should be consistent across distributed indexes
	verifyFTSDBSFResults(t, result.Hits)
}

// TestFTSDBSFNestedAliases tests DBSF with an alias containing two index aliases
func TestFTSDBSFNestedAliases(t *testing.T) {
	// Setup the nested aliases configuration
	masterAlias, cleanup := setupFTSNestedAliases(t)
	defer cleanup()
	...
	// Verify FTS DBSF results - should be consistent across nested aliases
	verifyFTSDBSFResults(t, result.Hits)
}
...
// TestFTSDBSFPagination tests FTS DBSF with pagination across different index/alias configurations
func TestFTSDBSFPagination(t *testing.T) {
	scenarios := []struct {
		name  string
		setup func(t *testing.T) (Index, func())
	}{
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			// Create first page request (first 5 results)
			firstPageRequest := createFTSSearchRequest(ScoreDBSF)
			firstPageRequest.From = 0
			firstPageRequest.Size = 5
			...
			// Create second page request (next 5 results, starting from index 5)
			secondPageRequest := createFTSSearchRequest(ScoreDBSF)
			secondPageRequest.From = 5
			secondPageRequest.Size = 5
			...
			// Verify combined FTS DBSF results
			verifyFTSDBSFResults(t, combinedHits)
		})
	}
}
...
// TestFTSDBSFFaceting tests that facet results are identical whether using DBSF or default scoring
func TestFTSDBSFFaceting(t *testing.T) {
	scenarios := []struct {
		name  string
		setup func(t *testing.T) (Index, func())
	}{
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			dbsfRequest := createFTSSearchRequest(ScoreDBSF)
			dbsfRequest.Size = 10
			// Add identical facet for category field with size 10
			categoryFacetDBSF := NewFacetRequest("category", 10)
			dbsfRequest.AddFacet("category", categoryFacetDBSF)
			...
			// Check that category facet exists in both results
			defaultCategoryFacet, defaultExists := defaultResult.Facets["category"]
			dbsfCategoryFacet, dbsfExists := dbsfResult.Facets["category"]

			if !defaultExists {
				t.Fatal("Expected category facet in default scoring results")
			}
			if !dbsfExists {
				t.Fatal("Expected category facet in DBSF scoring results")
			}

			// Compare the facet results - they should be identical
			// Since facets are based on the document corpus and not scoring,
			// they should not be affected by the scoring method
			if defaultCategoryFacet.Total != dbsfCategoryFacet.Total {
				t.Errorf("Facet totals differ: default=%d, DBSF=%d",
					defaultCategoryFacet.Total, dbsfCategoryFacet.Total)
			}

			if defaultCategoryFacet.Missing != dbsfCategoryFacet.Missing {
				t.Errorf("Facet missing counts differ: default=%d, DBSF=%d",
					defaultCategoryFacet.Missing, dbsfCategoryFacet.Missing)
			}

			if defaultCategoryFacet.Other != dbsfCategoryFacet.Other {
				t.Errorf("Facet other counts differ: default=%d, DBSF=%d",
					defaultCategoryFacet.Other, dbsfCategoryFacet.Other)
			}

			// Compare the facet terms
			defaultTerms := defaultCategoryFacet.Terms.Terms()
			dbsfTerms := dbsfCategoryFacet.Terms.Terms()

...
func TestGetBM25FieldNormTableConcurrent(t *testing.T) {
	// Verify no data race when multiple goroutines call getBM25FieldNormTable
	...
	for i := 0; i < 50; i++ {
		...
		go func(avg float64) {
			defer wg.Done()
			tbl := getBM25FieldNormTable(avg)
			if tbl == nil {
				t.Errorf("getBM25FieldNormTable returned nil")
			}
		}(avgDocLen)
	}
	wg.Wait()

...
// computeMAXSCOREPivot partitions maxscoreOrder (ascending by MaxImpact) into a
// non-essential prefix and an essential suffix at pivotIdx, maintaining
//
//	sum(MaxImpact of maxscoreOrder[:pivotIdx]) <= threshold
//
// with pivotIdx as large as that inequality allows.
//
// nextMAXSCORE never calls Next() on the non-essential searchers, so a document
// matching only those terms is never surfaced; the bound above is exactly what
// makes discarding it sound — such a document cannot score above the threshold
// even if it matches every non-essential term.
//
// The sum must therefore be over the PREFIX of the ascending order (the
// smallest impacts, which are the ones being dropped), not the suffix.  Summing
// the suffix instead moves the pivot the wrong way as the threshold falls —
// and the threshold falls as Size grows — which silently drops documents that
// belong in the top-k.
//
// When the whole sum fits under the threshold, pivotIdx = len(maxscoreOrder):
// no document can beat the threshold at all, which Next() reads as a signal to
// stop iterating and mark Total a lower bound.
func (s *DisjunctionSliceSearcher) computeMAXSCOREPivot(threshold float64) {
	s.lastThreshold = threshold
	var nonEssential float64
	pivot := 0
	for ; pivot < len(s.maxscoreOrder); pivot++ {
		next := nonEssential + s.wandMaxImpacts[s.maxscoreOrder[pivot]]
		if next > threshold {
			break
		}
		nonEssential = next

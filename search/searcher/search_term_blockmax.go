...
//
// BlockMax reports an upper bound on the reader's scoring contribution --
// highest term frequency, most favorable (shortest-field) norm factor -- for
...
type blockMaxReader interface {
	// BlockMax reports an upper bound on the reader's scoring contribution --
	// highest term frequency, most favorable (shortest-field) norm factor --
	// for every document up to and including lastDoc, without decoding
	// anything, plus how many documents that span covers. docCount lets a
	// caller that skips the span keep an exact hit count without knowing
	// anything about the segment's block size.
...
// -- see TermQueryScorer.CanScoreBulk), or the reader has nothing useful to
// report a bound for (a 1-hit term, a conjunction-narrowed iterator, the tail
// of a postings list, live deletions, or a segment implementation without
// this capability). In every such case the caller should just proceed with
// its normal fetch.
func (s *TermSearcher) skipUncompetitiveBlocks() error {
	if !s.hasMinCompetitiveScore || !s.scorer.CanScoreBulk() {
		return nil
	}
	...
	for {
		...
		if s.scorer.MaxScore(maxTF, maxNorm) > s.minCompetitiveScore {
			return nil
		}

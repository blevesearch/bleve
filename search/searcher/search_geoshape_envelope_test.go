...
func TestGeoJsonEnvelopeWithInQuery(t *testing.T) {
	tests := []struct {
		...
		{[][]float64{{76.256103515625, 16.76772739719064},
			{76.35772705078125, 16.872890378907783}},
			"geometry", []string{"point1"}},

		// test within query envelope for multipoint1.
		{[][]float64{{81.046142578125, 17.156537255486093},
...
func TestGeoJsonEnvelopeIntersectsQuery(t *testing.T) {
	tests := []struct {
		...
		{[][]float64{{81.00769042968749, 17.80622614478282},
	...
	for n, test := range tests {
		got, err := runGeoShapeEnvelopeRelationQuery("intersects",
			indexReader, test.points, test.field)
		if err != nil {
			t.Fatal(err)
		}
...
func setupGeoJsonShapesIndexForEnvelopeQuery(t *testing.T) index.Index {
	...
	polygon1 := [][][][]float64{{{{8.548071384429932, 47.379216780040124},
	...
	polygon2 := [][][][]float64{{{{76.70379638671874, 16.828203242420393},
		{76.36322021484375, 16.58881695544584}, {76.70928955078125, 16.720385051694},
	...
	polygon3 := [][][][]float64{{{{82.9522705078125, 17.749994573141873},
		{82.94952392578125, 17.692436998627272}, {82.87673950195312, 17.64009591883757},
		{82.76412963867188, 17.58643052828743}, {82.8094482421875, 17.522272941245202},
		{82.99621582031249, 17.64009591883757}, {82.9522705078125, 17.749994573141873}}}}
	...
	envelope1 := [][][][]float64{{{{74.89654541015625, 17.403062993328923},
	...
	linestring1 := [][][][]float64{{{{76.85211181640624, 17.51048642597462},
	...
	linestring2 := [][][][]float64{{{{81.89208984375, 18.555136195095105},
	...
	multipoint1 := [][][][]float64{{{{81.24938964843749, 17.602139123350838},
		{81.30432128906249, 17.56548361143177}, {81.29058837890625, 17.180155043474496},
	...
	multilinestring := [][][][]float64{{{{81.69708251953125, 17.641404631355755},
